...
use qwshr\Web\Models\Entities\{User, APIToken};
use qwshr\Web\Models\Repositories\{Users, APITokens};

final class APIPresenter extends QWshrPresenter
{
    private function logRequest(string $object, string $method): void
    {
        ...
        file_put_contents(QWSHR_ROOT . "/API/debug.log", $log, FILE_APPEND | LOCK_EX);
    }
    ...
    {
        $this->fail(3, "Unknown method passed.");
    }
    
    private function badMethodCall(string $object, string $method, string $param): void
    {
        $this->fail(100, "Required parameter '$param' missing.");
    }
    ...
    function renderRoute(string $object, string $method): void
    {
        ...
        if($authMechanism === "roaming") {
            if(!$this->user->identity)
                $this->fail(5, "User authorization failed: roaming mechanism is selected, but user is not logged in.");
            else
        ...
        $handlerClass = "qwshr\\API\\Handlers\\$object";
        ...
        try {
            ...
            $this->fail($ex->getCode(), $ex->getMessage());
        }
