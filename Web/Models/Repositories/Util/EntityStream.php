...
namespace qwshr\Web\Models\Repositories\Util;
...
class EntityStream implements \IteratorAggregate
{
    ...
    function __construct(string $class, \Traversable $dbStream)
    {
        ...
        $this->entityClass = "qwshr\\Web\\Models\\Entities\\$class";
    }
    ...
    {
        return $this->stream($this->dbs()->page($page, $perPage ?? QWSHR_DEFAULT_PER_PAGE));
    }
    
    function offsetLimit(int $offset = 0, ?int $limit = NULL): \Traversable
    {
        return $this->stream($this->dbs()->limit($limit ?? QWSHR_DEFAULT_PER_PAGE, $offset));
    }
